          ...
          <div className="mt-6 space-y-4">
            {screenName === SCREEN_WELCOME ? (
              <form className="space-y-4" onSubmit={onPasswordSubmit}>
                ...
                <div className="space-y-2"><label htmlFor="workspace-password" className="text-sm font-medium">Password</label><Input id="workspace-password" name="password" type="password" value={password} onChange={(event) => onPasswordChange(event.target.value)} placeholder="Password" autoComplete="current-password" required disabled={submitting} aria-invalid={Boolean(error)} className="h-11" /></div>
                <Button type="submit" className="h-10 w-full bg-blue-600 text-white hover:bg-blue-700" disabled={submitting}>Continue with password</Button>
